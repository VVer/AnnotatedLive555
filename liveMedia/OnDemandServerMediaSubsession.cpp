...
static void afterPlayingStreamState(void* clientData) {
	...
	///add by me
	//当文件播放完毕时，发送Announce命令
	portNumBits port = streamState->serverRTPPort().num();
	std::map<portNumBits,RTSPServer::RTSPConnectSession>::iterator iter = RTSPServer::fPortSessionMap.find(port);
	if (iter != RTSPServer::fPortSessionMap.end())
	{
		iter->second.SendAnnounce();
	}
	// end 
