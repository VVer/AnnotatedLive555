...
private:
	...
	char* fStreamName;	  //流的名字
	char* fInfoSDPString;	//对应于SDP info信息
	char* fDescriptionSDPString;	//对应于SDP 描述信息
	char* fMiscSDPLines;	//对应于SDP的其它信息
	struct timeval fCreationTime;	//当前ServerMediaSession创建的时间
	unsigned fReferenceCount;	//引用量
	Boolean fDeleteWhenUnreferenced;	//如果为真 没有引用的话，则调用析构函数
};
...
public:
	...
	virtual void getStreamParameters(unsigned clientSessionId, // in	 传入
		netAddressBits clientAddress, // in 传入
		Port const& clientRTPPort, // in	 传入
		Port const& clientRTCPPort, // in 传入
		int tcpSocketNum, // in (-1 means use UDP, not TCP) -1表示UDP，其他表示TCP
		unsigned char rtpChannelId, // in (used if TCP) 传入
		unsigned char rtcpChannelId, // in (used if TCP)	传入
		netAddressBits& destinationAddress, // in out   传入传出
		u_int8_t& destinationTTL, // in out	传入传出
		Boolean& isMulticast, // out 传出/返回
		Port& serverRTPPort, // out 传出/返回
		Port& serverRTCPPort, // out 传出/返回
		void*& streamToken // out 传出/返回
		) = 0;
	...
	//这个函数用来定位到相对时间（如NPT）。
	//参数“streamDuration“，如果大于0.0, 指定多长时间的数据要发送，通过传递参数 "seekNPT"。（如果小于等于0.0，则说明所有的数据已经传完了）
	//参数"numBytes" 返回将要传输的数据的大小，0表示未知或者是不限。
	...
	//这个函数用来定位到绝对时间。
	//absStart的格式应该是这样的字符串“YYYYMMDDTHHMMSSZ”或者是“YYYYMMDDTHHMMSS.<frac>Z”。
	//absEnd应该是NULL（对于未知结束时间的，如直播流），或者是同absStart
	//这些字符串有可能被修改，或者是被一个新的值替代。
...
private:
	...
	friend class ServerMediaSubsessionIterator;
	ServerMediaSubsession* fNext;	//指向下一个ServerMediaSubsession
